...
I'll be using bzminer here, so I'm focusing on the file [here](misc/hive-templates/gor_bz_local_adapter.json). Hive gives the option to import from a file or from your clipboard; I'll be copying directly from the clipboard in this example. 
...
Last step, you just need to edit the custom miner section with the address of the gor node you're mining to. This needs the full ip/port so it'll look similar to `192.168.0.12:16110` save the changes after you're finished
...
Ideally you'd run the bridge on the same machine that your gord node is running on. You can follow any of the installation methods defined in the main readme of the repo.
