...
func CleanWallet(in string) (string, error) {
	_, err := util.DecodeAddress(in, util.Bech32Prefixgor)
	if err == nil {
	...
	// has gor: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:65], nil
	}
	return "", errors.New("unable to coerce wallet to valid gor address")
}
