...
import (
	...
	"path"
	"time"
	"github.com/onemorebsmith/gorstratum/src/gorstratum"
	"gopkg.in/yaml.v2"
	"db"
)
...
func main() {
	...
	flag.StringVar(&cfg.RPCServer, "gor", cfg.RPCServer, "address of the gord node, default `localhost:26110`")
	...
	if cfg.BlockWaitTime == 0 {
		cfg.BlockWaitTime = 5 * time.Second // this should never happen due to gor 1s block times
	}
	log.Println("----------------------------------")
	log.Printf("initializing bridge")
	log.Printf("\tgord:          %s", cfg.RPCServer)
	...
	if err := gorstratum.ListenAndServe(cfg); err != nil {
		log.Println(err)
	}
}
